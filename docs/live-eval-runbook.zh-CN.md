...
这份 Runbook 的面试价值不是“会设置环境变量”，而是证明：

- 真实模型评测具有固定环境、固定 Commit 和版本化 Rubric。
- 凭据不进入代码、Trace 和公开证据。
- 冒烟、正式运行、失败审计和成本核算有明确顺序。
- 旧结果不会被新结果覆盖，指标变化可以追溯。

超过这些内容的云端 Secret Manager、组织权限和生产部署不属于当前项目范围，不继续扩展。
